...
export async function prepareCollectionRun(options: CollectorJobOptions = {}): Promise<PreparedRun | null> {
  ...
  try {
    ...
    if (activeCheck.rows.length > 0) {
      console.warn(
        `Collection run requested while database run ${activeCheck.rows[0].run_id} is active (started at ${activeCheck.rows[0].started_at.toISOString()}). Rejecting concurrent run.`
      );
      return null;
    }
  } catch (err) {
...
export async function executeCollectionRunFromPrepared(
  ...
  try {
    ...
    // 3. Spawn Python process and parse line-by-line streaming JSON progress
    ...
    try {
      const errJson = JSON.stringify([err.message || String(err)]);
      await query(
...
export async function persistCollectionPayload(
  ...
  try {
    ...
    // 4. Persist each collected session snapshot inside PostgreSQL atomic transactions
